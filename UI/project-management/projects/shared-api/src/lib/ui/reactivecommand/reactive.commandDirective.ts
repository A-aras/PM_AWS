...
@Directive({
	selector: "[reativeCommand]",
	exportAs: "reativeCommand"
})
export class ReactiveCommandDirective implements OnInit, OnDestroy {

	@Input("reativeCommand") commandOrCreator: IReactiveCommand | IReactiveCommandCreator | undefined;

	/** @deprecated Use `commandInput` instead. */
	// @Input("command")
	// get _commandOrCreator() { return this.commandOrCreator; }
	// set _commandOrCreator(value: IReactiveCommand | IReactiveCommandCreator | undefined) {
	...
	@Input("reativeCommandParams") commandParams: any | any[];
	...
	ngOnInit() {
		...
		if (!this.commandOrCreator) {
			throw new Error("reativeCommand: [reativeCommand] should be defined!");
			...
			this._command = new ReactiveCommand(execFn, this.commandOrCreator.canExecute, isAsync);
		} else {
			throw new Error("reativeCommand: [reativeCommand] is not defined properly!");
		}
