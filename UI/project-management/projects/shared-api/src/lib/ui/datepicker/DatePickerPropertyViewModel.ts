...
export class DatePickerPropertyViewModel extends PropertyViewModel {
  ...
  constructor(
    ...
    propFunc: (model: IReactiveClassModel) => any
  ) {
    super(viewModel);
...
//   private ApplyFilter(
//     value: MasterDataModel | string,
//     items: MasterDataModel[]
//   ): MasterDataModel[] {
//     try {
//       let searchValue = "";
...
//     let domainItems = store
//       .pipe(select(this.PropertyRule.Enumaration.Selector))
//       .pipe(
//         map(items => {
//           let domainItem: MasterDataModel[] = [];
//           let mappedValue = items.map(item => {
//             let masterData = new MasterDataModel();
//             masterData.DisplayMember = this.PropertyRule.Enumaration.DisplayValue(
//               item
//             );
//             masterData.ValueMember = this.PropertyRule.Enumaration.ValuePath(
//               item
//             );
...
//         let formChange$ = this.Form.valueChanges.pipe(
//           startWith(""),
//           map(x => {
//             return x;
//           })
...
//         formChange$
//           .pipe(
//             combineLatest(enumarationChange$, (x, y) => {
